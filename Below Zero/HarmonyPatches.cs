...
namespace SubnauticaAutosave_BZ
{
    ...
    {
        private const string modName = "Subnautica Autosave BZ";
        ...
        private static bool Patch_ManualSaveGame_Prefix()
        {
            ...
            if (controller != null)
            {
                if (Config.ConfigComprehensiveSaves)
                {
        ...
        {
            if (HarmonyPatches.Config.ConfigShowSaveNames && SaveLoadManager.main.GetGameInfo(lb.saveGame) != null)
            {
        ...
        {
#if DEBUG
            HarmonyPatches.LogMessage("Main scene loading, scheduling first autosave.");
#endif

            Player.main?.GetComponent<AutosaveController>()?.ScheduleAutosave(HarmonyPatches.Config.ConfigMinutesBetweenAutosaves);
        }

        // Untested //
        private static void Patch_Bed_OnHandClick_Postfix()
        {
            if (HarmonyPatches.Config.ConfigAutosaveOnSleep)
            {
                ...
                if (player != null)
                {
                    ...
                    {
#if DEBUG
                        HarmonyPatches.LogMessage("Player clicked on bed. Executing save on sleep.");
#endif
        ...
        {
#if DEBUG
            HarmonyPatches.LogMessage("Player entered or exited sub. Delaying autosave.");
#endif
        ...
        public static void InitializeHarmony()
        {
            ...
            Harmony harmony = new Harmony("Dingo.Harmony.SubnauticaAutosaveBZ");
